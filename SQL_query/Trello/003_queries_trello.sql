...
WHERE ot.OwnerTypeValue = 'Workspace' AND u.Id = 1;
...
SELECT TOP 7 Id, TemplateCategoryName , IconUrl
...
WHERE TC.TemplateCategoryName ='VP Marketing'

-- Get information templates(category = 'VP Marketing', id=22) (Slide 7)
SELECT t.Id, t.Title, u.PictureUrl, t.TemplateDescription, t.Copied, t.Viewed, t.BackgroundUrl
FROM Templates t
JOIN TemplateCategories tc ON tc.Id = t.TemplateCategoryId
JOIN Users u ON u.Id = t.CreatedBy
WHERE tc.TemplateCategoryName ='VP Marketing' AND t.Id ='22'
...
WHERE tc.TemplateCategoryName = 'Financial Analyst'
ORDER BY 
    t.Viewed DESC
...
FROM Boards b
    ...
    JOIN Users u ON u.Id = m.UserId
WHERE ot.OwnerTypeValue = 'Board' AND u.Id = 1 AND w.Id = 1
...
SELECT pu.Id, pu.PowerUpName, pu.PowerUpDescription, pu.IconUrl, pu.AuthorName, pu.BackgroundUrl, puc.PowerUpCategoryName
FROM PowerUps pu
JOIN PowerUpCategories puc ON pu.PowerUpCategoryId = puc.Id
WHERE puc.id = 2
...
JOIN  Users u ON u.Id = a.UserId
WHERE u.Id = 2 AND n.NotificationStatus = 'UNREAD'
...
WHERE ac.UserId = 2 AND n.NotificationStatus = 'UNREAD';
