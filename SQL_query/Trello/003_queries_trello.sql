...
FROM Labels l
JOIN CardLabel cl ON cl.LabelId = l.Id
...
FROM Activity a
JOIN OwnerType ot ON ot.Id = a.OwnerTypeId
...
FROM 
  ...
  AND ct.Id = 6 -- Get top 3 most viewed templates. (Slide 5)
SELECT 
  TOP 3 t.Id, 
  t.Title, 
...
FROM 
  Templates t 
ORDER BY 
  Viewed DESC -- Search templates by keyword in title. (Slide 5)
SELECT 
  t.Id, 
  t.Title, 
  t.BackgroundUrl, 
